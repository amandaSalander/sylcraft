...
void BossLabel::render(const Position &position, SDL_Renderer *gRenderer) {
    ...
    if( !textTexture.loadFromRenderedText(name, textColor,gRenderer,gFont ) )
    {
        std::cout <<"Failed to load TEXT ! SDL_ttf Error: "<<std::endl;
    }
    else {
        textTexture.render(position.getX()+2,position.getY()+18, nullptr,0.0, nullptr,SDL_FLIP_NONE,gRenderer);
    }
